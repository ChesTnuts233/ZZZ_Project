...
public static class ExtralEditorUtility
{
	...
	public static void CreateListView(this ListView createdListView, VisualTreeAsset item, IList itemsSource, SelectionType selectionType = SelectionType.Multiple)
	{
		Func<VisualElement> makeItem = () => item.Instantiate();

		Action<VisualElement, int> bindItem = (visualElement, index) =>
		{
			//这里是绑定 Item  如果越界或者非法就去除元素并且返回
		...
		createdListView.selectionType = selectionType;
