...
namespace KooFrame.BaseSystem
{
    ...
    public static class ScriptsTemplatesCreater
    {
        ...
        /// 添加路径信息到字典中
        /// </summary>
        /// <param name="codeData">代码数据</param>
        /// <param name="targetPath">目标路径</param>
        public static void AddDicInDirectoryData(CodeData codeData, string targetPath)
        {
            string directoryPath = Path.GetDirectoryName(targetPath);
            string dataPath = directoryPath + "/" + Path.GetFileName(directoryPath) + ".asset";
            if (!File.Exists(dataPath)) //如果存在
            {
                return;
            }

            DirectoryData treeData = AssetDatabase.LoadAssetAtPath<DirectoryData>(dataPath);
            treeData.PathDic.Dic.Add(codeData, targetPath);
        }
