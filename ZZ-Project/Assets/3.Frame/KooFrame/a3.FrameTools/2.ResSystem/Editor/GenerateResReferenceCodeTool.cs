...
{
    static string scriptPath = Application.dataPath + "/3.Frame/Res.cs";
    ...
    public static void GenerateResReferenceCode()
    {
        ...
        foreach (AddressableAssetGroup group in assets.groups)
        {
            ...
            for (int i = 0; i < allAssetEntry.Count; i++)
            {
                ...
                {
                    string subAssetPropertyStr =
                        SubAssetPropertyTemplate.Replace("##类型##", assetItem.MainAssetType.Name);
                    if (assetItem.MainAssetType.Name == "AudioMixerGroupController")
                        subAssetPropertyStr = PropertyTemplate.Replace("##类型##", "AudioMixerGroup");
                    if (assetItem.MainAssetType.Name == "AudioMixerSnapshotController")
                        subAssetPropertyStr = PropertyTemplate.Replace("##类型##", "AudioMixerSnapshot");
                ...
                else
                {
                    ...
                    //特殊情况处理
                    if (assetItem.MainAssetType.Name == "AudioMixerController")
                        propertyStr = PropertyTemplate.Replace("##类型##", "AudioMixer");
                    ...
                    {
                        string gameObjectPropertyStr =
                            GameObjectPropertyTemplate.Replace("##资源名称##", assetItem.address.Replace(" ", ""));
