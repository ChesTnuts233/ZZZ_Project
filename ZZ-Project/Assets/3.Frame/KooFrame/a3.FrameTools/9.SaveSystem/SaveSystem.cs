...
namespace KooFrame
{
    ...
    public static class SaveSystem
    {
        ...
        private static void SaveFile(object saveObject, string path)
        {
            switch (FrameRoot.Setting.SaveSystemType)
            {
                case SaveSystemType.Binary:
                    ...
                    {
                        KooTool.SaveFile(saveObject, path);
                    }
                    else
                    {
                ...
                case SaveSystemType.Json:
                    ...
                    KooTool.SaveJson(jsonData, path);
                    break;
            }
        }
        ...
        //     {
        //         case SaveSystemType.Binary:
        //             return KooTool.LoadFile<T>(path);
        //         case SaveSystemType.Json:
        //             return KooTool.LoadJson<T>(path);
        //     }
        //
        //     return null;
        // }
        ...
        static string assetsPath = UnityEngine.Application.dataPath;

        static int lastSlashIndex = assetsPath.LastIndexOf("/Rage Legend/Assets", StringComparison.Ordinal);

        static string parentPath = UnityEngine.Application.streamingAssetsPath + "/";

        #endregion

        public static void SaveModel(object saveModel)
        {
            string dirPath = parentPath + saveModel;
            string jsonData = JsonUtility.ToJson(saveModel);
            KooTool.SaveJsonWithUnescape(jsonData, dirPath);
            Debug.Log("保存Model数据成功");
        }


        public static T LoadModel<T>() where T : class
        {
            T model = KooTool.LoadJson<T>(parentPath + typeof(T).FullName + ".json");

            return model;
        }

        public static string LoadModelJson<T>() where T : class
        {
            return File.ReadAllText(parentPath + typeof(T).FullName + ".json");
        }
